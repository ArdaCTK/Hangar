...
const Sidebar: React.FC<Props> = ({ onSettingsClick, onDashboardClick }) => {
  const {
    projects,
    selectedProject,
    setSelectedProject,
    ...
    <div className="sidebar">
      ...
      <div className="sidebar-nav">
        <button
          ...
          onClick={() => { setSelectedProject(null); onDashboardClick(); }}
        >
      ...
      <div className="sidebar-projects">
        {filtered.map((p) => (
          ...
          >
            <span
              className="lang-dot"
              style={{ background: getColor(p.languages[0] ?? "") }}
            />
            <span className="sidebar-project-name">{p.name}</span>
            {p.has_git && (
              <svg width="9" height="9" viewBox="0 0 16 16" fill="currentColor" style={{ opacity: 0.4, flexShrink: 0 }}>
        ...
        {filtered.length === 0 && !isScanning && (
          <div style={{ padding: "20px 10px", color: "var(--text3)", fontSize: "12px", textAlign: "center" }}>
            No projects found
          </div>
        )}
      </div>
      ...
      <div className="sidebar-footer">
        <button className="sidebar-settings-btn" onClick={onSettingsClick}>
          <svg width="13" height="13" viewBox="0 0 16 16" fill="none">
            ...
            <path
              d="M8 1v1.5M8 13.5V15M15 8h-1.5M2.5 8H1M12.95 3.05l-1.06 1.06M4.11 11.89l-1.06 1.06M12.95 12.95l-1.06-1.06M4.11 4.11l-1.06-1.06"
