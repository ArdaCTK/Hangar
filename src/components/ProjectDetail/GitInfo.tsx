...
const GitInfo: React.FC<Props> = ({ details }) => {
  ...
  const { git_log, git_branches, git_current_branch } = details;
    ...
    <div>
      ...
      {tab === "local" && (
        <div>
          ...
          <div className="section-block">
            <div className="section-block-title">Branches ({git_branches.length})</div>
            <div className="branch-list">
              {git_branches.map((b) => (
                <span
                  key={b}
                  className={`branch-tag ${b.trim() === git_current_branch ? "current" : ""}`}
                >
                  {b.trim() === git_current_branch && "● "}
                  {b.trim()}
                </span>
              ))}
            ...
            <>
              ...
              <div className="info-grid">
                <div className="info-card">
                  <div className="info-card-label">Stars</div>
                  <div className="info-card-value yellow">★ {ghData.stars.toLocaleString()}</div>
                </div>
                <div className="info-card">
                  <div className="info-card-label">Forks</div>
                  <div className="info-card-value blue">{ghData.forks.toLocaleString()}</div>
                </div>
                <div className="info-card">
                  <div className="info-card-label">Open Issues</div>
                  <div className="info-card-value orange">{ghData.open_issues.toLocaleString()}</div>
                </div>
                <div className="info-card">
                  <div className="info-card-label">Open PRs</div>
                  <div className="info-card-value purple">{ghData.open_prs.toLocaleString()}</div>
                </div>
                <div className="info-card">
                  <div className="info-card-label">Watchers</div>
                  <div className="info-card-value">{ghData.watchers.toLocaleString()}</div>
                </div>
                <div className="info-card">
                  <div className="info-card-label">Size</div>
                  <div className="info-card-value" style={{ fontSize: "14px" }}>{ghData.size_kb >= 1024 ? `${(ghData.size_kb / 1024).toFixed(1)} MB` : `${ghData.size_kb} KB`}</div>
                </div>
              </div>
              ...
              {ghData.topics.length > 0 && (
                <div className="section-block">
                  ...
                  <div className="branch-list">
                    {ghData.topics.map((t) => (
                      <span key={t} className="branch-tag">{t}</span>
                    ))}
                  </div>
                </div>
              )}
              ...
              <div style={{ marginTop: "12px", display: "flex", gap: "16px" }}>
                <span style={{ fontSize: "11px", color: "var(--text3)" }}>
                  Created {ghData.created_at.slice(0, 10)}
                </span>
                <span style={{ fontSize: "11px", color: "var(--text3)" }}>
                  Updated {ghData.updated_at.slice(0, 10)}
                </span>
                {ghData.license_name && (
                  <span style={{ fontSize: "11px", color: "var(--text3)" }}>
                    License: {ghData.license_name}
                  </span>
                )}
              </div>
            </>
