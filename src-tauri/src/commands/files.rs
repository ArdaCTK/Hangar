    ...
    if is_dir && depth < MAX_TREE_DEPTH {
        if let Ok(rd) = std::fs::read_dir(path) {
            ...
            for entry in entries.into_iter().take(MAX_CHILDREN_PER_DIR) {
                ...
                if child_name.starts_with('.') && child_name != ".env"
                    && !child_name.starts_with(".env.")
                    && child_name != ".gitignore"
                    && child_name != ".github"
                {
                    // allow common dotfiles but skip .git, .svelte-kit etc
                    if child_path.is_dir()
                        && SKIP_DIRS.iter().any(|s| *s == child_name.as_str())
                    {
                        continue;
                    }
                }
...
pub fn get_file_tree(path: String) -> Result<Vec<FileNode>, String> {
    ...
    for entry in entries.into_iter().take(MAX_CHILDREN_PER_DIR) {
        ...
        if p.is_dir() && SKIP_DIRS.iter().any(|s| *s == n.as_str()) {
            continue;
        }
...
pub fn get_project_details(path: String) -> Result<ProjectDetails, String> {
    ...
    // Re-scan to get fresh ProjectInfo
    ...
    let git_log = if info.has_git {
        git_cmds::get_git_log(project_path, 50)
    } else {
        vec![]
    };
